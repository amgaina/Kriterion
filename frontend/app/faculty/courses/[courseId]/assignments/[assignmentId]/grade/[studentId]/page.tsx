...
export default function GradingPage() {
    ...
    const runCode = async () => {
        ...
        // Build files from loaded contents
        ...
        try {
            // If professor selected specific test cases, map to test_case_ids
                        ...
                        <div className="w-56 bg-[#252526] border-r border-[#3c3c3c] flex flex-col min-h-0 shrink-0">
                            ...
                            <div className="px-2 py-1">
                                <div className="flex items-center gap-1 px-2 py-1 text-[11px] text-[#cccccc]">
                                    ...
                                    <span className="font-medium truncate">Attempt #{selectedSub.attempt_number}</span>
                                </div>
                            </div>
                                        ...
                                        {subFiles.map(file => (
                                            <div key={file.id} onClick={() => selectFile(file.id)}
                                                className={`flex items-center gap-2 px-2 py-1 rounded cursor-pointer text-[12px] ${
                                                    selectedFileId === file.id ? 'bg-[#862733]/30 text-white' : 'text-[#cccccc] hover:bg-[#2a2d2e]'
                                                }`}>
                            ...
                            <div className="border-t border-[#3c3c3c]">
                                <div className="px-4 py-2 text-[11px] font-semibold uppercase tracking-wider text-[#bbbbbb]">
                                    All Attempts ({studentSubs.length})
                                </div>
                                <div className="overflow-y-auto max-h-40 px-2 pb-2 space-y-0.5">
                                    {studentSubs.map((sub, idx) => (
                                        <div key={sub.id} onClick={() => setSelectedSubId(sub.id)}
                                            className={`flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer text-[11px] ${
                                                selectedSubId === sub.id ? 'bg-[#862733]/30 text-white' : 'text-[#cccccc] hover:bg-[#2a2d2e]'
                                            }`}>
                                            <span className="font-semibold">#{sub.attempt_number}</span>
                                            {idx === 0 && <span className="text-[9px] px-1 bg-[#862733]/20 text-[#862733] rounded">Latest</span>}
                                            <span className="flex-1 truncate text-[#858585]">
                                                {format(new Date(sub.submitted_at), 'MMM dd')}
                                            </span>
                                            <span className={`font-semibold ${getScoreColor(sub.final_score, sub.max_score)}`}>
                                                {sub.final_score !== null ? sub.final_score.toFixed(0) : '—'}
                                            </span>
                                        </div>
                                    ))}
                                </div>
                                                    ...
                                                    <div className="space-y-3">
                                                        ...
                                                        <div className={`flex items-center gap-2 p-2.5 rounded-lg ${
                                                            ...
                                                            <span className={`font-semibold ${
                                                                ...
                                                                : 'text-[#f44747]'
                                                            }`}>{runResult.compilation_status || (runResult.success ? 'Compiled Successfully' : 'Not Compiled Successfully')}</span>
                                                            {runResult.results.length > 0 && (
                                                                <span className="ml-auto text-[10px] text-[#858585]">{runResult.tests_passed}/{runResult.tests_total} tests passed</span>
                                                            )}
                                                        </div>

                                                        {/* Full compiler/runtime message */}
                                                        {runResult.message && (
                                                            <div>
                                                                <p className="text-[10px] text-[#858585] uppercase tracking-wider mb-1">Compiler / Runtime Output</p>
                                                                <pre className={`whitespace-pre-wrap text-[11px] leading-relaxed p-3 rounded-lg border ${
                                                                    runResult.compilation_status === 'Compiled Successfully'
                                                                        ? 'bg-[#1a1a2e] border-[#3c3c3c] text-[#d4d4d4]'
                                                                        : 'bg-[#2d0000] border-[#5c1e1e] text-[#f44747]'
                                                                }`}>{runResult.message}</pre>
                                                            </div>
                                                        )}
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 ${
                                                            selectedTestCases.has(tr.id) ? 'bg-[#862733] border-[#862733] text-white' : 'border-[#505050]'
                                                        }`}>
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
                        <div className="bg-[#252526] border border-[#3c3c3c] rounded-2xl p-8 max-w-sm w-full mx-4 text-center shadow-2xl animate-in fade-in zoom-in-95 duration-300">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    onClick={() => router.push(`/faculty/courses/${courseId}/assignments/${assignmentId}`)}
