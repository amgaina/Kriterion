    ...
    const filteredEvents = useMemo(() => {
        if (selectedDate) {
            return events.filter((e) => {
                try { return isSameDay(parseISO(e.date), selectedDate); }
                catch { return false; }
            });
        }
...
function eventStyle(event: FacultyEvent) {
...
function eventHref(event: FacultyEvent) {
...
/* Events list for desktop sidebar */
function EventsList({ events, selectedDate }: { events: FacultyEvent[]; selectedDate: Date | null }) {
    return (
